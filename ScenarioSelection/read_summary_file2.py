...
def read_pwc_output(in_file):
    # Read the summary output table, manually entering in the header (original header is tough to parse)
    ...
    data.columns = ['reg', 'crop', 'run_id', 'hl_id', 'scenario_id', 'rep']
    # assigned 'run_id' to the koc class ('koc_id') portion of the line batch run ID field (could also concatenate)
...
def main():
    ...
    scenario_table = os.path.join("Input", "Samples", "r05_1_Corn_sample.csv")  # table used as input for the pwc run
    summary_file = os.path.join("Input", "Samples", "r05_Corn_K10_HL0_Summary_SW.txt")  # output file from the pwc run
    output_dir = "Output"  # where output data will be written
    summary_output = os.path.join(output_dir, "r05_Corn_K10_HL0_summary.csv")
    selection_output = os.path.join(output_dir, "r05_Corn_K10_HL0_selection.csv")
